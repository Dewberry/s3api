...
// Validate X-S3API-User-Email header against user from claims
func (kas *KeycloakAuthStrategy) ValidateUser(c echo.Context, claims *Claims) (err error) {
	...
	} else if claims.Email == "" || c.Request().Header.Get("X-S3API-User-Email") != claims.Email {
		fmt.Println(claims.Email)
		fmt.Println(c.Request().Header.Get("X-S3API-User-Email"))
		return fmt.Errorf("invalid X-S3API-User-Email header")
	}
...
func (kas *KeycloakAuthStrategy) SetUserRolesHeader(c echo.Context, claims *Claims) (err error) {
	...
	if exists {
		...
		c.Request().Header.Set("X-S3API-User-Roles", rolesString)
	} else {
		return nil
	}
